    ...
    getRulesFromTargetChain :: Packet -> RWMonad [Rule]
    getRulesFromTargetChain p = do
        env <- ask
        let fwIP = firewallIP env
            chainsList = rulesChains env
            target = if dstip p == fwIP
                    then Input
                    else if srcip p == fwIP
                        then Output
                        else Forward
            matches = filter (\(pt, rs) -> target == pt) chainsList
        
        logMsg Information ("MATCHES:::" `T.append` (T.pack $ Prelude.show matches)) (Just p)
        if (length matches /= 1) 
            then do 
                    logMsg Error "Multiples definiciones de cadena" (Just p)
                    return []
    ...
    processPacket p = do
        ...
        if (not sbpbf)
            then do 
                    ...
                    chain <- getRulesFromTargetChain p
                    evalChain chain p
